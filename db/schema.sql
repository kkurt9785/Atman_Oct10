...
CREATE TABLE workers (
  ...
  -- 18+ 게이트
  CONSTRAINT chk_age_18 CHECK (birth_date <= CURRENT_DATE - INTERVAL '18 years')
);
...
CREATE INDEX idx_shifts_open ON shifts(shift_date, status)
  WHERE status = 'open' AND shift_date >= CURRENT_DATE;
...
RETURNS TABLE (
  ...
  WHERE w.deleted_at IS NULL
    ...
    AND ST_DWithin(
      w.activity_center,
      p_facility_location,
      LEAST(w.activity_radius_meters, p_max_distance_meters)
    )
