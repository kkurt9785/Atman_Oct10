...
export function BasicInfo({ birthDate, attendanceInvite = false, submitting = false, submitError = '', onNext }: { birthDate: string; attendanceInvite?: boolean; submitting?: boolean; submitError?: string; onNext: (value: BasicInfoValue) => void }) {
  ...
  return (
    <div className="flex flex-col min-h-screen px-6 pt-14 pb-10">
      ...
      <h1 className="text-[28px] font-bold text-ink letter-tight mb-2">{attendanceInvite ? '초대받은 정보를 확인해요' : '본인 정보를 입력해주세요'}</h1>
      <p className="text-[15px] text-sub mb-8">{attendanceInvite ? '관리자가 등록한 이름과 휴대폰 번호를 똑같이 입력해 주세요.' : '지원자 확인과 급여 지급에 사용돼요.'}</p>
      ...
      <p className="text-[13px] text-tertiary">{attendanceInvite ? '초대 링크의 연락처 끝 4자리와 같은 번호여야 연결할 수 있어요.' : '공고 지원·채용 확정·급여 상태 알림을 받을 번호예요.'}</p>
