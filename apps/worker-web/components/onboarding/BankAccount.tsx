...
const BANKS = [
  { code: '090', name: '카카오뱅크' }, { code: '092', name: '토스뱅크' }, { code: '089', name: '케이뱅크' },
  { code: '088', name: '신한' }, { code: '004', name: 'KB국민' },
  { code: '081', name: '하나' }, { code: '020', name: '우리' },
  { code: '011', name: 'NH농협' }, { code: '003', name: 'IBK기업' },
  { code: '045', name: '새마을금고' }, { code: '048', name: '신협' }, { code: '071', name: '우체국' }, { code: '007', name: '수협' },
  { code: '023', name: 'SC제일' }, { code: '027', name: '씨티' }, { code: '002', name: '산업' },
  { code: '031', name: 'iM뱅크(대구)' }, { code: '032', name: '부산' }, { code: '039', name: '경남' },
  { code: '034', name: '광주' }, { code: '037', name: '전북' }, { code: '035', name: '제주' },
] as const;
    ...
    <div className="flex flex-col min-h-screen px-6 pt-14 pb-10">
      ...
      <div className="relative mb-4">
        ...
        <button type="button" aria-labelledby="bank-label bank-value" aria-haspopup="listbox" aria-expanded={showDropdown} aria-controls="bank-options" onClick={() => setShowDropdown((value) => !value)} className="w-full h-[52px] flex items-center justify-between px-4 bg-white rounded-card border border-line">
          <span id="bank-value" className={`text-[16px] ${selected ? 'text-ink font-medium' : 'text-tertiary'}`}>{selected?.name ?? '은행 선택'}</span><span aria-hidden="true">⌄</span>
        </button>
        {showDropdown && <div id="bank-options" role="listbox" aria-labelledby="bank-label" className="absolute top-full left-0 right-0 mt-1 bg-white rounded-card shadow-card border border-line z-10 overflow-hidden">{BANKS.map((bank) => <button type="button" role="option" aria-selected={bank.code === bankCode} key={bank.code} onClick={() => { setBankCode(bank.code); setShowDropdown(false); }} className="w-full px-4 py-3.5 text-left text-[16px] text-ink hover:bg-bg border-b border-line last:border-0">{bank.name}</button>)}</div>}
      </div>
