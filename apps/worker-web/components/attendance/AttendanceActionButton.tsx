...
export function AttendanceActionButton({targetType,targetId,action,qrToken,onSuccess}:{
  ...
  return <div className="mt-3">
    ...
    {result&&<div role="status" className={`mt-2 rounded-xl p-3 text-[12px] font-bold ${result.ok?'bg-emerald-50 text-emerald-700':'bg-red-50 text-red-600'}`}>
      ...
      {result.ok&&<><p className="mt-1 font-medium">{new Date(result.checkOutAt??result.checkInAt??Date.now()).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit',hour12:false})} · {METHOD[result.method??'']??result.method}{typeof result.distanceM==='number'?` · 병원에서 ${result.distanceM}m`:''}{typeof result.accuracyM==='number'?` · 위치 오차 ±${Math.round(result.accuracyM)}m`:''}</p><button onClick={()=>window.location.reload()} className="mt-2 h-9 rounded-lg bg-white px-3 text-[12px] font-extrabold text-emerald-700">완료</button></>}
    ...
    <p className="mt-2 text-center text-[11px] text-sub">위치는 출퇴근 버튼을 누른 순간에만 확인해요.</p>
  </div>;
}
