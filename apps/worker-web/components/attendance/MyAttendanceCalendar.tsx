...
const AUTH: Record<string, string> = { GPS: '위치', GPS_QR: '위치+QR', QR: 'QR', QR_FALLBACK: 'QR 보완', ADMIN: '관리자' };
...
const LEGEND: [string, string][] = [['bg-primary', '완료'], ['bg-primary/15', '근무 중'], ['bg-amber-400', '지각'], ['bg-red-500', '결근'], ['bg-emerald-400', '휴가']];
  ...
  return (
    <div>
      <div className="flex items-center justify-between rounded-xl bg-bg p-1.5">
        ...
        <b className="text-[13px]">{month.replace('-', '년 ')}월 · 완료 {doneDays}일 · {Math.floor(totalMin / 60)}시간 {totalMin % 60}분</b>
        {month < current
      ...
      {loading ? <p className="mt-4 rounded-xl bg-bg p-4 text-center text-[12px] text-sub">근태를 불러오는 중...</p> : <>
        ...
        {rows.length === 0
          ? <p className="mt-3 rounded-xl bg-bg p-4 text-center text-[12px] text-sub">{month.replace('-', '년 ')}월 근태 기록이 없어요.</p>
