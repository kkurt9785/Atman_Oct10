...
export default async function StaffPage() {
  const [staff, pending] = await Promise.all([getStaff(), getPendingWorkers()]);
  ...
  return (
    <main className="px-4">
      ...
      <SectionTitle>워커 승인 대기</SectionTitle>
      {pending.length === 0 ? (
        <Card className="py-8 text-center">
          <p className="text-body font-bold text-ink">승인 대기 워커가 없어요</p>
          <p className="text-label text-sub mt-1">신규 가입자가 심사를 마치면 여기에 표시됩니다.</p>
        </Card>
      ) : (
