...
export default async function StaffPage({searchParams}:{searchParams:Promise<{view?:string;entry?:string}>}) {
  ...
  const gigworkerMode=shop?.facilityType==='gigworker';
  ...
  return <main className="px-4 pb-28">
    <div className="mt-3 mb-4 px-1">
      ...
      <p className="text-label text-sub mt-1">{gigworkerMode?'근무 일정만 등록하고 링크나 QR로 가입시켜요. 서로 전화번호를 몰라도 됩니다.':`직원 10명 이하 ${facilityWord}도 설정 없이 바로 시작할 수 있어요.`}</p>
    </div>
    ...
    <div className={`${gigworkerMode?'grid-cols-1':'grid-cols-2'} grid gap-2 mt-3`}>
      ...
      {!gigworkerMode&&<Link href="/leave" className="min-h-tap rounded-xl bg-white border border-line flex items-center justify-center text-body font-bold">휴가 관리</Link>}
    </div>

    <details open={gigworkerMode||isGigworkerEntry} className="mt-5 bg-white rounded-2xl shadow-card group">
      <summary className="list-none cursor-pointer px-5 py-4 flex items-center justify-between">
        <span className="font-bold text-body">＋ {gigworkerMode||isGigworkerEntry?'긱워커 등록·초대':'기존 직원 직접 등록'}</span><span className="text-sub group-open:rotate-180">⌄</span>
      </summary>
      ...
      <StaffRegistrationForm facilityType={shop?.facilityType} initialEngagementType={isGigworkerEntry?'temporary':undefined} workerOrigin={workerOrigin}/>
    </details>

    {view!=='shift'&&<><SectionTitle>{gigworkerMode?'등록한 긱워커':view==='regular'?'상시 직원':view==='contract'?'계약·임시 직원':'관리 중인 직원'}</SectionTitle>
