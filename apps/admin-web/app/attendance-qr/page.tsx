...
export default async function AttendanceQrPage(){
  ...
  return <main className="px-4 pb-28">
    ...
    <section className="mt-5 print:hidden">
      ...
      <div className="mt-3 rounded-xl border border-line bg-white p-4"><p className="text-[13px] font-bold text-ink">단기 워커가 본인 QR을 보여주나요?</p><p className="mt-1 text-[12px] leading-5 text-sub">이 경우는 관리자가 카메라로 워커의 일회용 QR을 스캔하는 방식이에요.</p><Link href="/checkin" className="mt-3 inline-flex h-10 w-full items-center justify-center rounded-xl border border-primary/25 text-[12px] font-bold text-primary">워커 QR 스캔 열기</Link></div>
    </section>
