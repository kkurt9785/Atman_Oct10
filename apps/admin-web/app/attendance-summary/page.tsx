...
function isIssue(status:string,earlyLeaveMinutes:number){return ['late','absent','checkout_pending'].includes(status)||earlyLeaveMinutes>0;}
...
export default async function AttendanceSummaryPage({searchParams}:{searchParams:Promise<{month?:string}>}){
  ...
  const people=[...rows.reduce((map,row)=>{
    ...
    person.issues+=isIssue(row.status,row.earlyLeaveMinutes)?1:0;
  ...
  return <main className="px-4 pb-28">
    ...
    <div className="mt-3 grid grid-cols-3 gap-2">
      ...
      <Card className="p-3"><p className="text-[11px] text-sub">인정 근무</p><b className="mt-1 block text-title">{Math.floor(totalMinutes/60)}h</b></Card>
