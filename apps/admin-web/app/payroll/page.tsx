...
export default async function PayrollPage({searchParams}:{searchParams:Promise<{month?:string}>}) {
  ...
  return <main className="px-4">
    <div className="mt-3 mb-5 px-1"><p className="text-label font-bold text-primary">{facilityWord} 직접 지급</p><h1 className="text-display font-extrabold text-ink">급여 지급관리</h1><p className="text-label text-sub mt-2">근무 기록을 승인하고 {facilityWord} 계좌에서 워커에게 직접 지급하세요. 잇닿은 임금을 보관하지 않습니다.</p><div className="mt-3 flex gap-2"><Link href={`/attendance-history?month=${selectedMonth}`} className="inline-flex h-10 flex-1 items-center justify-center rounded-xl border border-primary/30 bg-white text-primary text-label font-bold">근태 확인·마감</Link><a href={`/api/payroll/export?month=${selectedMonth}`} className="inline-flex h-10 flex-1 items-center justify-center rounded-xl border border-primary/30 bg-white text-primary text-label font-bold">급여 CSV</a></div></div>
    ...
    : <div className="space-y-3">{rows.map(row => <Card key={row.id} className={row.status==='disputed'?'border border-red-200':''}>
      ...
      <div className="mt-4 rounded-xl bg-bg p-3 space-y-2 text-label"><div className="flex justify-between"><span className="text-sub">세전 예상액</span><b>{won(row.grossAmount)}</b></div><div className="flex justify-between"><span className="text-sub">공제 상태</span><b>{row.deductionStatus==='unconfirmed'?'미확정 · 사업장 확인 필요':'사업장 확인'}</b></div><div className="flex justify-between border-t border-line pt-2"><span>지급 예정액</span><b className="text-primary">{won(row.netAmount)}</b></div>{row.dueDate&&<div className="flex justify-between"><span className="text-sub">지급 예정일</span><b>{row.dueDate}</b></div>}</div>
    ...
    <p className="text-[13px] text-sub px-1 mt-4 leading-5">CSV는 검토·회계 전달용이며 계좌번호는 끝 4자리만 포함합니다. 실제 계좌 이체는 사업장이 직접 실행하고, 지급완료 표시는 실제 이체 확인 후 처리하세요. 모든 상태 변경은 감사로그에 기록됩니다.</p>
  </main>;
}
