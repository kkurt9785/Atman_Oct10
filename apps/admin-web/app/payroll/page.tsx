...
export default async function PayrollPage({searchParams}:{searchParams:Promise<{month?:string}>}) {
  ...
  return <main className="px-4">
    ...
    : <div className="space-y-3">{rows.map(row => <Card key={row.id} className={row.status==='disputed'?'border border-red-200':''}>
      ...
      {row.bankName&&row.accountLast4&&<p className="mt-2 rounded-lg bg-blue-50 p-3 text-[12px] font-bold text-blue-700">워커 등록 지급계좌 · {row.bankName} ****{row.accountLast4}</p>}
      {row.disputeReason&&<p className="mt-3 rounded-lg bg-red-50 p-3 text-label text-red-600">워커 확인 요청: {row.disputeReason}</p>}
      {canManage&&row.status==='draft'&&(selectedMonth<currentMonth||shop.isDemo)&&<PayrollActionForm kind="marketplace" action="approve" values={{id:row.id}} label="금액 검토 후 지급 승인" className="w-full h-11 rounded-xl bg-primary text-white text-label font-extrabold"/>}
