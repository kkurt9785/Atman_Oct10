...
export default async function PayrollPage({searchParams}:{searchParams:Promise<{month?:string}>}) {
  ...
  const [shop, paymentResult, staffRows, context] = await Promise.all([getShop(), getWagePayments(selectedMonth), getStaffWagePayments(selectedMonth), getAdminContext()]);
  ...
  return <main className="px-4">
    <div className="mt-3 mb-5 px-1"><p className="text-label font-bold text-primary">병원 직접 지급</p><h1 className="text-display font-extrabold text-ink">급여 지급관리</h1><p className="text-label text-sub mt-2">근무 기록을 승인하고 병원 계좌에서 워커에게 직접 지급하세요. 잇닿은 임금을 보관하지 않습니다.</p><div className="mt-3 flex gap-2"><Link href={`/attendance-history?month=${selectedMonth}`} className="inline-flex h-10 flex-1 items-center justify-center rounded-xl border border-primary/30 bg-white text-primary text-label font-bold">근태 확인·마감</Link><a href={`/api/payroll/export?month=${selectedMonth}`} className="inline-flex h-10 flex-1 items-center justify-center rounded-xl border border-primary/30 bg-white text-primary text-label font-bold">급여 CSV</a></div></div>
    ...
    <div className="mb-3 mt-6 flex items-end justify-between px-1"><div><p className="text-[12px] font-bold text-primary">병원 등록 인력 + 공고 지원 인력</p><h2 className="text-title font-extrabold">{selectedMonth.replace('-','년 ')}월 급여</h2></div><div className="flex items-center gap-1 rounded-xl bg-white p-1 shadow-sm"><Link href={`/payroll?month=${moveMonth(selectedMonth,-1)}`} className="flex h-8 w-8 items-center justify-center rounded-lg text-lg" aria-label="이전 달">‹</Link><span className="min-w-20 text-center text-[12px] font-bold">{selectedMonth.replace('-','년 ')}월</span>{selectedMonth<currentMonth?<Link href={`/payroll?month=${moveMonth(selectedMonth,1)}`} className="flex h-8 w-8 items-center justify-center rounded-lg text-lg" aria-label="다음 달">›</Link>:<span className="flex h-8 w-8 items-center justify-center text-line" aria-hidden>›</span>}</div></div>
    {selectedMonth===currentMonth&&<Card className="mb-3 border border-blue-100 bg-blue-50"><p className="text-label font-bold text-ink">이번 달은 집계 중이에요</p><p className="mt-1 text-[12px] text-sub">금액은 중간 예상치이며 지급 승인은 다음 달 마감 후 가능합니다.</p></Card>}
    {staffRows.length===0?<Card className="py-8 text-center"><p className="font-bold">등록된 직원이 없어요</p><a href="/staff" className="mt-2 inline-block text-label font-bold text-primary">직원 등록하기 →</a></Card>:<div className="space-y-3">{staffRows.map(row=><Card key={row.staffId}>
      ...
      {row.status==='config_required'?<a href="/staff" className="mt-4 flex h-11 items-center justify-center rounded-xl bg-amber-50 text-label font-bold text-amber-700">직원관리에서 급여 기준 설정</a>:<>
        <div className="mt-4 rounded-xl bg-bg p-3 text-label"><div className="flex justify-between"><span className="text-sub">완료 근태</span><b>{row.workedDays}일 · {Math.floor(row.workedMinutes/60)}시간 {row.workedMinutes%60}분</b></div><div className="mt-2 flex justify-between border-t border-line pt-2"><span>{row.needsProration?'기준 월급':'세전 예상액'}</span><b className="text-primary">{won(row.grossAmount)}</b></div></div>
        {row.needsProration&&<p className="mt-2 rounded-lg bg-amber-50 p-3 text-[12px] font-bold text-amber-700">일할계산 필요 · {row.prorationReason}</p>}
        {!row.accountReady&&<p className="mt-2 rounded-lg bg-red-50 p-3 text-[12px] font-bold text-red-600">지급 전 은행과 계좌 끝 4자리를 등록해 주세요.</p>}
        {canManage&&row.status==='draft'&&selectedMonth<currentMonth&&<PayrollActionForm kind="staff" action="approve" values={{staff_id:row.staffId,period_month:row.periodMonth}} needsAmount={row.needsProration} label="근태·금액 검토 후 지급 승인" className="h-11 w-full rounded-xl bg-primary text-label font-extrabold text-white"/>}
        {canManage&&row.status==='approved'&&<PayrollActionForm kind="staff" action="mark_exported" values={{staff_id:row.staffId,period_month:row.periodMonth}} label="이체 준비 완료 표시" className="h-11 w-full rounded-xl bg-ink text-label font-extrabold text-white"/>}
        {canManage&&row.status==='exported'&&<PayrollActionForm kind="staff" action="mark_paid" values={{staff_id:row.staffId,period_month:row.periodMonth}} label="병원 지급 완료 표시" className="h-11 w-full rounded-xl bg-success text-label font-extrabold text-white"/>}
      </>}
    </Card>)}</div>}
    ...
    : <div className="space-y-3">{rows.map(row => <Card key={row.id} className={row.status==='disputed'?'border border-red-200':''}>
      ...
      {canManage&&row.status==='draft'&&selectedMonth<currentMonth&&<PayrollActionForm kind="marketplace" action="approve" values={{id:row.id}} label="금액 검토 후 지급 승인" className="w-full h-11 rounded-xl bg-primary text-white text-label font-extrabold"/>}
      ...
      {canManage&&row.status==='exported'&&<PayrollActionForm kind="marketplace" action="mark_paid" values={{id:row.id}} label="병원 지급 완료 표시" className="w-full h-11 rounded-xl bg-success text-white text-label font-extrabold"/>}
    </Card>)}</div>}
