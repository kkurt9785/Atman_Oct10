...
export default async function MorePage(){
  ...
  const isPlatform=isPlatformAdminEmail(context?.user.email);
  return <main className="px-4"><div className="px-1 mt-2 mb-5"><p className="text-label font-bold text-primary">사업장 관리</p><h1 className="mt-1 text-display font-extrabold text-ink">무엇을 관리할까요?</h1><p className="mt-2 text-body text-sub">기준 총원과 근무 기준을 잡고, 부족 인원과 반복 공백을 여기에서 관리해요.</p></div>
    {isPlatform&&<Link href="/ops/facilities" className="mb-3 block active:opacity-80"><Card className="flex items-center gap-4 border border-primary/30 p-5"><div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-primary text-white"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 20V8l8-4 8 4v12"/><path d="M9 20v-6h6v6"/><path d="m8 11 2 2 4-4"/></svg></div><div className="min-w-0 flex-1"><p className="text-label font-bold text-primary">잇닿 운영</p><h2 className="mt-0.5 text-[18px] font-extrabold text-ink">사업장 승인</h2><p className="mt-1 text-label leading-5 text-sub">셀프 등록 사업장 확인·승인, 수동 등록 요청 검토</p></div><span className="text-sub">›</span></Card></Link>}
