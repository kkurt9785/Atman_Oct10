...
export function validateBrnDocument(file: File): string | null {
  if (!BRN_DOC_MIME_EXT[file.type]) return '사진(JPG·PNG·HEIC) 또는 PDF만 올릴 수 있어요.';
...
export async function uploadBrnDocument(file: File, facilityId: string): Promise<string> {
  ...
  const path = `${user.id}/${facilityId}/brn-${Date.now()}.${BRN_DOC_MIME_EXT[file.type]}`;
  const { error: uploadError } = await supabase.storage.from('facility-documents').upload(path, file, { cacheControl: '3600', upsert: false, contentType: file.type });
