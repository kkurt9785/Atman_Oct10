...
export async function getAttendanceHistory(month:string){
  ...
  for(const raw of (shifts??[]) as any[]){const person=Array.isArray(raw.workers)?raw.workers[0]:raw.workers;const shift=Array.isArray(raw.shifts)?raw.shifts[0]:raw.shifts;const worked=raw.check_in_at&&raw.check_out_at?Math.max(0,Math.round((new Date(raw.check_out_at).getTime()-new Date(raw.check_in_at).getTime())/60000)):0;rows.push({id:raw.id,kind:'shift',staffId:null,name:person?.name??'단기 워커',employment:'shift',role:person?.role??'',workDate:shift?.shift_date,scheduledStart:shift?.start_time,scheduledEnd:shift?.end_time,checkInAt:raw.check_in_at,checkOutAt:raw.check_out_at,breakMinutes:0,workedMinutes:worked,status:raw.check_out_at?'completed':raw.check_in_at?'working':'scheduled',method:raw.check_out_method??raw.check_in_method,lateMinutes:0,earlyLeaveMinutes:0,correctionReason:null});}
