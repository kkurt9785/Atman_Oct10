...
export async function getAttendanceHistory(month:string){
  ...
  const [staffResult,shiftResult,closureResult]=await Promise.all([
    ...
    sb.from('shift_attendances').select('id,check_in_at,check_out_at,checkout_requested_at,check_in_method,check_out_method,late_minutes,early_leave_minutes,workers(name,role),shifts!inner(facility_id,shift_date,start_time,end_time)')
  ...
  const loadError=[staffResult.error,shiftResult.error,closureResult.error].find(Boolean);
  if(loadError)throw new Error(`월 근태 내역을 불러오지 못했어요: ${loadError.message}`);
  ...
  for(const raw of (shifts??[]) as any[]){const person=Array.isArray(raw.workers)?raw.workers[0]:raw.workers;const shift=Array.isArray(raw.shifts)?raw.shifts[0]:raw.shifts;const worked=raw.check_in_at&&raw.check_out_at?Math.max(0,Math.round((new Date(raw.check_out_at).getTime()-new Date(raw.check_in_at).getTime())/60000)):0;rows.push({id:raw.id,kind:'shift',staffId:null,name:person?.name??'단기 워커',employment:'shift',role:person?.role??'',workDate:shift?.shift_date,scheduledStart:shift?.start_time,scheduledEnd:shift?.end_time,checkInAt:raw.check_in_at,checkOutAt:raw.check_out_at,breakMinutes:0,workedMinutes:worked,status:raw.check_out_at?'completed':raw.checkout_requested_at?'checkout_pending':raw.check_in_at?'working':'scheduled',method:raw.check_out_method??raw.check_in_method,lateMinutes:Number(raw.late_minutes??0),earlyLeaveMinutes:Number(raw.early_leave_minutes??0),correctionReason:null});}
