...
**구독 규약**: `push_subscriptions`는 `worker_id + endpoint` 단위로 저장한다. 같은 관리자 계정의 휴대폰·PC는 모두 알림을 받고, 한 기기에서 알림을 끄더라도 다른 기기 구독은 유지된다. 구독이 없거나 만료되어 푸시가 `discarded`된 알림도 앱 내 알림함에는 남는다.
...
8. **관리자 두 기기** → 휴대폰과 PC에서 모두 알림 허용 → 워커 지원 1건 → 두 기기 모두 수신, 한 기기에서만 알림 해제 후 다른 기기 수신 유지 확인
